...
EOF
  elif [[ "${RUNTIME_MODE}" == "apple_foundation_models" ]]; then
    cat <<'EOF'
iOS manual hints:
- Open /Users/amanpandey/projects/ross/ios/Ross.xcodeproj in Xcode on a compatible Apple Intelligence device.
- Follow docs/MANUAL_LOCAL_INFERENCE_QA.md for the explicit opt-in flow and technical-details checks.
- Confirm that the last invocation runtime shows apple_foundation_models before claiming a real run.
EOF
  else
