...
usage() {
  cat <<'EOF'
Usage:
  scripts/ios-device-assistant-download-smoke.sh --device <device> --tier <tier> [options]
...
def validate_identity_guard(identity):
    ...
    if diagnostic_error:
        print(
            "ROSS_ASSISTANT_DOWNLOAD_SMOKE_GUARD_FAIL "
            f"reason=runtime_identity_diagnostic_error requested={expected_identity_runtime} "
            f"{diagnostic_error}",
            file=sys.stderr,
        )
        sys.exit(1)
