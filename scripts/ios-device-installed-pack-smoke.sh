...
def validate_identity_guard(identity, *, require_identity):
    ...
    if require_identity:
        ...
        if diagnostic_error:
            print(
                "ROSS_SMOKE_GUARD_FAIL "
                f"reason=runtime_identity_diagnostic_error requested={runtime_mode} "
                f"{diagnostic_error}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def emit_failure_identity_diagnostic_guard(identity, *, outcome):
    ...
    if identity is None:
        print(
            "ROSS_SMOKE_GUARD_FAIL "
            f"reason=missing_runtime_identity_{suffix} requested={runtime_mode}",
            file=sys.stderr,
        )
        return
    ...
    if availability_error:
        print(
            "ROSS_SMOKE_GUARD_FAIL "
            f"reason=runtime_identity_unavailable_{suffix} requested={runtime_mode} "
            f"{availability_error}",
            file=sys.stderr,
        )
        return
    ...
    if artifact_error:
        print(
            "ROSS_SMOKE_GUARD_FAIL "
            f"reason=runtime_identity_artifact_mismatch_{suffix} requested={runtime_mode} "
            f"{artifact_error}",
            file=sys.stderr,
        )
        return
    ...
    if resource_error:
        print(
            "ROSS_SMOKE_GUARD_FAIL "
            f"reason=runtime_identity_resource_missing_{suffix} requested={runtime_mode} "
            f"{resource_error}",
            file=sys.stderr,
        )
        return
    ...
    if diagnostic_error:
        print(
            "ROSS_SMOKE_GUARD_FAIL "
            f"reason=runtime_identity_diagnostic_error_{suffix} requested={runtime_mode} "
            f"{diagnostic_error}",
            file=sys.stderr,
        )
