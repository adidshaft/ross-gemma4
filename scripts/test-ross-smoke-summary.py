...
class RossSmokeSummaryTests(unittest.TestCase):
    ...
    def test_benchmark_summary_rejects_coreai_directory_adapter_file_identity(self):
        identity = self.valid_identity("apple_foundation_models")
        identity["provider"] = "AlphaFoundationModelsLocalProvider"
        identity["model_format"] = "coreml_model"
        identity["artifact_path_type"] = "file"
        identity["artifact_path"] = "FoundationAdapter.mlmodelc"
        identity["draft_status"] = "not_supported"
        with self.assertRaisesRegex(
            MissingBenchmarkMatrixError,
            "benchmark_runtime_artifact_mismatch adapter_path_type=file",
        ):
            benchmark_summary_line(
                identity,
                {
    ...
    def test_runtime_identity_artifact_rules_reject_wrong_lane_shapes(self):
        ...
        self.assertEqual(
            ...
            "mlx_directory_path=model.safetensors",
        )
        self.assertIsNone(
            runtime_identity_artifact_error(
                {
    ...
    def test_runtime_identity_draft_artifact_rules_are_lane_aware(self):
        ...
        self.assertEqual(
            runtime_identity_draft_artifact_error(
                {**active_mlx, "draft_model": "draft.safetensors"},
                "mlx_swift_lm",
            ),
    ...
    def test_benchmark_summary_rejects_impossible_draft_acceptance_telemetry(self):
        identity = parse_fields(
            "ROSS_RUNTIME_IDENTITY provider=AlphaLlamaCppProvider "
            "requested_runtime=gemma_local_runtime actual_runtime=gemma_local_runtime "
            "pack_runtime=gemma_local_runtime "
        ...
        valid_pass_fields = parse_fields(
            "ROSS_LOCAL_MODEL_SMOKE_PASS runtime=gemma_local_runtime requested_runtime=gemma_local_runtime profile=mtp_quick elapsed=10.00s "
            "source_input_tokens=120 source_output_tokens=8 source_token_speed=11.0 "
            "source_first_token_ms=900 source_measured_tokens=true "
            "source_refs=1 source_native_model=true "
            "source_acceleration=draftModelSpeculative source_draft_tokens=2 source_draft_model=mtp.gguf "
            "source_draft_attempted=4 source_draft_accepted=3"
        )
        ...
        invalid_cases = [
            ...
            ("source_draft_attempted=nil source_draft_accepted=1", "source_draft_attempted=nil"),
        ]
        for draft_fields, expected_error in invalid_cases:
            with self.subTest(draft_fields=draft_fields):
                pass_fields = parse_fields(
                    "ROSS_LOCAL_MODEL_SMOKE_PASS runtime=gemma_local_runtime requested_runtime=gemma_local_runtime profile=mtp_quick elapsed=10.00s "
                    "source_input_tokens=120 source_output_tokens=8 source_token_speed=11.0 "
                    "source_first_token_ms=900 source_measured_tokens=true "
    ...
    def test_benchmark_summary_rejects_mtp_profile_without_active_draft_identity(self):
        ...
        pass_fields = parse_fields(
            ...
            "source_first_token_ms=900 source_measured_tokens=true "
            "source_refs=1 source_native_model=true "
            "source_acceleration=standard source_draft_tokens=nil source_draft_model=nil "
    ...
    def test_failure_summary_preserves_identity_errors_and_stage_metrics(self):
        ...
        fail_fields = parse_fields(
            ...
            "runtime_error_detail=stage_runtime_failed draft_error_detail=stage_draft_inactive "
        ...
        self.assertIn("fail_runtime_error_detail=stage_runtime_failed", summary)
        self.assertIn("fail_draft_error_detail=stage_draft_inactive", summary)
        self.assertIn("matrix_profile=full", summary)
    ...
    def test_failure_summary_rejects_fallback_runtime_identity(self):
        identity = self.valid_identity("mlx_swift_lm")
        identity.update(
            {
                "provider": "AlphaMLXLocalProvider",
                "fallback": "deterministic_dev",
            }
        )
        fail_fields = parse_fields(
            "ROSS_LOCAL_MODEL_SMOKE_FAIL runtime=mlx_swift_lm requested_runtime=mlx_swift_lm "
            "profile=quick stage=source error=source_fact_missing elapsed=3.00s"
        )
        ...
        self.assertIn("failure_runtime_proof_status=runtime_identity_invalid", summary)
        self.assertIn("failure_runtime_proof_error=runtime_unavailable", summary)
        self.assertIn("fallback=deterministic_dev", summary)
        self.assertIn("identity_availability_error=fallback=deterministic_dev", summary)

    def test_failure_summary_marks_mtp_fallback_identity_invalid(self):
        identity = self.valid_identity()
        identity.update(
            {
                "acceleration": "draftModelSpeculative",
        ...
        self.assertIn("failure_runtime_proof_status=runtime_identity_invalid", summary)
        self.assertIn("failure_runtime_proof_error=runtime_unavailable", summary)
        self.assertIn("failure_mtp_proof_status=runtime_identity_invalid", summary)
        self.assertIn("failure_mtp_proof_error=runtime_unavailable", summary)
        self.assertIn("identity_availability_error=fallback=deterministic_dev", summary)
    ...
    def test_failure_summary_survives_missing_runtime_identity(self):
        fail_fields = parse_fields(
            ...
            "stage=runtime_health error=missing_runtime_identity "
            "runtime_error_detail=missing_runtime_identity draft_error_detail=missing_runtime_identity "
            "elapsed=2.4s"
        )
        ...
        self.assertIn("fail_runtime_error_detail=missing_runtime_identity", summary)
        self.assertIn("fail_draft_error_detail=missing_runtime_identity", summary)
    ...
    def test_failure_summary_uses_fail_requested_runtime_when_identity_is_nil(self):
        ...
        fail_fields = parse_fields(
            "ROSS_LOCAL_MODEL_SMOKE_FAIL runtime=mlx_swift_lm requested_runtime=mlx_swift_lm "
            "profile=quick stage=active_pack error=missing_mlx_artifact "
            "runtime_error_detail=missing_mlx_artifact draft_error_detail=no_draft_configured "
            "elapsed=2.4s"
        )
