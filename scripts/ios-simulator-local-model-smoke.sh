...
def validate_identity_guard(identity, *, require_identity):
    ...
    if pack_runtime not in (None, "nil", actual_runtime):
        print(
            "ROSS_SMOKE_GUARD_FAIL "
            f"reason=pack_runtime_mismatch requested={runtime} "
    ...
    if require_identity:
        ...
        if diagnostic_error:
            print(
                "ROSS_SMOKE_GUARD_FAIL "
                f"reason=runtime_identity_diagnostic_error requested={runtime} "
                f"{diagnostic_error}",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        expected_artifact = (
            ...
            else os.path.basename(model_path.rstrip("/"))
        )
        ...
        if artifact_kind == "system_model":
            if identity_artifact != "system-model" and not identity_artifact.startswith("system://"):
                print(
                    "ROSS_SMOKE_GUARD_FAIL "
                    f"reason=runtime_identity_artifact_path_mismatch requested={runtime} "
