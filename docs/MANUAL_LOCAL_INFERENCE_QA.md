...
- For seeded MTP proof, add `--draft-model <local-draft.gguf> --draft-tokens 2 --smoke-profile mtp_quick --require-draft-acceleration`. The helper copies the draft GGUF into the same run-scoped tier directory, writes it as `draftArtifact` in the seeded manifest, and fails unless the app reports active `draftModelSpeculative` identity plus per-stage draft metrics.
