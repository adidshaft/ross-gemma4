...
- Simulator MTP proof also requires `--draft-model`; the helper now fails before launch if `--require-draft-acceleration` is requested without a draft artifact path, if GGUF/MTP proof receives a non-GGUF, wrong-header, or tiny placeholder draft artifact, if MLX draft proof receives a file-like GGUF/bin artifact or malformed MLX directory, or if CoreAI/CoreML asks for draft proof. As of June 19, 2026, `/Users/amanpandey/model-artifacts/mtp-gemma-4-12b-it.gguf` is locally present and checksum-verified, but the simulator MTP rerun still ended with `ROSS_SMOKE_GUARD_FAIL reason=no_terminal_smoke_marker outcome=None`; no simulator MTP acceleration number is claimed from this checkpoint.
