...
- GGUF identity markers now include `draft_status` so standard-generation MTP attempts are diagnosable instead of ambiguous. Expected values include `active`, `no_draft_configured`, `draft_file_unavailable`, `draft_format_unsupported`, `draft_token_policy_blocked`, `validator_rejected`, `validator_failed`, and `runtime_unavailable`.
