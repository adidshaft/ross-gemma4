...
- If the main GGUF runtime is available but draft validation fails, the identity marker stays `available=true` for the baseline runtime and reports a draft-specific `error` such as `draft_validator_rejected`, `draft_validator_failed`, `draft_token_policy_blocked`, `draft_format_unsupported`, or `draft_file_unavailable`. Treat those as failed MTP proof, not failed GGUF proof.
