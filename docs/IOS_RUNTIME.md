...
- Benchmark summaries now require every matrix stage to carry `*_input_tokens`, `*_output_tokens`, `*_token_speed`, `*_first_token_ms`, and `*_measured_tokens`. Missing or internally inconsistent stage metrics are rejected as `benchmark_stage_metrics_missing`, so `Tokens processed` and `Token speed` are benchmark evidence only when tied to a matrix stage and verified runtime identity.
