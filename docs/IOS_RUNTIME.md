...
| GGUF llama.cpp runtime | `AlphaLlamaCppEngine` and `AlphaLlamaCppProvider` | Simulator smoke passed on June 2, 2026 with a local GGUF developer artifact. The project is now pinned to `llama.swift` `2.9670.0` as of June 16, 2026, which resolves the newer upstream `llama.cpp` Apple XCFramework build `b9670`, and the GGUF lane continues to expose speculative draft acceleration for Gemma 4 MTP companions while the visible 3-pack ladder points at the newer Unsloth QAT GGUF repos. |
