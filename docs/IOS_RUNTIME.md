...
- After identity guardrails pass, the cabled-device smoke helpers print `ROSS_SMOKE_BENCHMARK_SUMMARY` with the verified runtime identity, acceleration metadata, profile, elapsed time, and any stage token metrics from the pass line. Use that summary for benchmark tables instead of scraping unguarded app logs.
...
- Assistant-download smokes also fail closed for explicit runtime requests: the selected terminal job and the installed pack must match the requested runtime, so an MLX/CoreAI download smoke cannot pass from a previously installed GGUF pack.
