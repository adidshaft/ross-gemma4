...
Private AI Pack downloads are Gemma 4 Q4 model files for the assistant tiers. Matter Search uses a separate embedding model. None of those model files are committed to the repo or bundled in the base app.
...
Current production-intended direction is `gemma_local_runtime` for Gemma 4 generative tiers plus a dedicated embedding model for retrieval. `deterministic_dev` remains the offline-safe test fallback.
