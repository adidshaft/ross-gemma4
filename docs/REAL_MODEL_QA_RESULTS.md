...
- Observed behavior:
  - this rebuilt simulator app honored the then-current `mtp_quick` cap: the context was `2048`, prompt batch was `256`, physical batch was `128`, and the two-stage matrix kept `max_tokens=24`; the current repo proof lane is now tighter at `context_tokens=1024`, prompt batch `128`, and physical batch `64`
...
- Observed behavior:
  ...
  - At that checkpoint, new `mtp_quick` runs used a smoke-only `2048` context cap with `256` prompt batch and `128` physical batch for the main model; the current repo proof lane is tighter at `context_tokens=1024`, prompt batch `128`, and physical batch `64`.
