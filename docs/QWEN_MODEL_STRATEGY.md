# Gemma 4 E2B Q4 Model Strategy

Ross uses a Gemma 4 E2B Q4-first local generative stack for Private AI Pack tiers and a separate embedding model for local retrieval.
...
## Why Gemma 4 E2B Q4 Gemma 4 Q4

- Gemma 4 Q4 gives Ross one practical local model packaging path across Android, iOS, desktop, and backend metadata.
- Gemma 4 E2B Q4 has compact tiers that match Ross's private-assistant ladder without forcing a very large first download.
...
Ross should not rely on the generative Gemma 4 E2B Q4 model alone for retrieval.
...
- Display name: Matter Search
- Technical model: Gemma 4 Embedding
- Repo: `Gemma 4 E2B Q4/Gemma-4-Embedding`
...
- Backend `ROSS_MODEL_CATALOG_MODE=production_metadata` advertises Gemma 4 E2B Q4 metadata but does not serve large model files unless a future delivery path is configured.
...
- Hardware proof of the full Gemma 4 E2B Q4 stack on target devices.
