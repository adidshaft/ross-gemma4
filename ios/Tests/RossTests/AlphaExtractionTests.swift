...
final class AlphaExtractionTests: XCTestCase {
    ...
    @MainActor
    func testAutomaticMLXDraftEnvironmentReducesDraftTokensAfterSlowRecentMLXRun() async throws {
        let store = AlphaRossStore()
        await store.removeAllModelArtifacts()

        let quickStartSource = try makeMLXDirectoryFixture(named: "ross-mlx-quickstart-slow-\(UUID().uuidString)")
        let caseAssociateSource = try makeMLXDirectoryFixture(named: "ross-mlx-caseassociate-slow-\(UUID().uuidString)")
        defer {
            try? FileManager.default.removeItem(at: quickStartSource)
            try? FileManager.default.removeItem(at: caseAssociateSource)
        }
        ...
        XCTAssertEqual(runtimeEnvironment.draftModelTokens, 4)
    }

    @MainActor
    func testAutomaticMLXDraftEnvironmentRaisesDraftTokensAfterFastRecentMLXRun() async throws {
        let store = AlphaRossStore()
        await store.removeAllModelArtifacts()

        let quickStartSource = try makeMLXDirectoryFixture(named: "ross-mlx-quickstart-fast-\(UUID().uuidString)")
        let caseAssociateSource = try makeMLXDirectoryFixture(named: "ross-mlx-caseassociate-fast-\(UUID().uuidString)")
        defer {
            try? FileManager.default.removeItem(at: quickStartSource)
            try? FileManager.default.removeItem(at: caseAssociateSource)
        }
    ...
    func testReleaseReadyAssistantArtifactsPinDownloadMetadata() throws {
        let expected: [AlphaCapabilityTier: (
            ...
            .flash: (
                ...
                remoteFileName: "google_gemma-4-E2B-it-Q2_K.gguf",
        ...
        for tier in AlphaCapabilityTier.installableAssistantTiers {
            ...
            XCTAssertEqual(
                ...
                "https://huggingface.co/\(pinned.repository)/resolve/main/\(pinned.remoteFileName)"
            )
            XCTAssertTrue(
    ...
    func testRuntimeHealthRejectsNonGGUFDraftBeforeValidator() throws {
        actor StubLlamaContext: AlphaLlamaCompletionContext {
            func clear() {}
            func completionInit(
                text: String,
        ...
        let mainURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("ross-gguf-main-non-gguf-draft-\(UUID().uuidString)")
            .appendingPathExtension("gguf")
        let draftURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("ross-gguf-draft-wrong-format-\(UUID().uuidString)")
        ...
        AlphaLlamaCppProvider.draftAccelerationValidator = { _, _, _ in
            XCTFail("Non-GGUF draft candidates should fail before llama draft validation.")
            return true
        }
        defer {
            AlphaLlamaCppProvider.contextFactory = previousContextFactory
    ...
    func testRuntimeHealthReportsGGUFDraftValidatorFailureWithoutDisablingBaselineRuntime() throws {
        ...
        let mainURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("ross-gguf-main-throwing-draft-\(UUID().uuidString)")
            .appendingPathExtension("gguf")
        let draftURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("ross-gguf-draft-throwing-\(UUID().uuidString)")
            .appendingPathExtension("gguf")
        try Data("gguf-main-runtime".utf8).write(to: mainURL)
        try Data("gguf-draft-runtime".utf8).write(to: draftURL)
        defer {
    ...
    func testExperimentalGGUFProviderSurfacesDefaultDraftTokensWhenImplicit() async throws {
        ...
        actor StubLlamaContext: AlphaLlamaCompletionContext {
            ...
            func completionLoop() -> String { "Implicit draft answer" }
            func isDone() -> Bool { true }
            func promptTokenCount() -> Int { 512 }
            func generatedTokenCount() -> Int { 18 }
            func accelerationMode() -> AlphaLocalRuntimeAccelerationMode { .draftModelSpeculative }
            func executionPathLabel() -> String { "Gemma GGUF with draft acceleration" }
        }

        let mainURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("ross-gguf-main-default-draft-\(UUID().uuidString)")
            .appendingPathExtension("gguf")
        let draftURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("ross-gguf-draft-default-\(UUID().uuidString)")
            .appendingPathExtension("gguf")
        try Data("gguf-main-runtime".utf8).write(to: mainURL)
        try Data("gguf-draft-runtime".utf8).write(to: draftURL)
        defer {
        ...
        let capture = DraftCapture()
        AlphaLlamaCppProvider.contextFactory = { _, _, draftTokens in
            capture.record(draftTokens: draftTokens)
            return StubLlamaContext()
        }
        AlphaLlamaCppProvider.modelLoadValidator = { _ in }
        AlphaLlamaCppProvider.draftAccelerationValidator = { _, _, _ in true }
    ...
    func testExperimentalGGUFProviderAllowsLowTokenDraftAccelerationForConstrainedQuickStartE4B() async throws {
        final class DraftCapture: @unchecked Sendable {
            ...
            func recordValidator(draftPath: String, draftTokens: Int?) {
                lock.lock()
                defer { lock.unlock() }
                self.validatorDraftPath = draftPath
                self.validatorDraftTokens = draftTokens
            }
        }
        ...
        AlphaLlamaCppProvider.contextFactory = { _, draftPath, draftTokens in
            capture.record(draftPath: draftPath, draftTokens: draftTokens)
            return StubLlamaContext()
        }
        AlphaLlamaCppProvider.modelLoadValidator = { _ in }
        ...
        XCTAssertEqual(capture.validatorDraftPath, draftURL.path)
        XCTAssertEqual(capture.validatorDraftTokens, 2)
        XCTAssertEqual(capture.draftPath, draftURL.path)
        XCTAssertEqual(capture.draftTokens, 2)
        XCTAssertEqual(health.accelerationMode, .draftModelSpeculative)
        XCTAssertEqual(health.accelerationDraftTokens, 2)
        XCTAssertEqual(health.draftModelPathLabel, draftURL.lastPathComponent)
    ...
    func testExperimentalGGUFProviderSuppressesHighTokenDraftAccelerationForConstrainedQuickStartE4B() async throws {
        ...
        AlphaLlamaCppProvider.modelLoadValidator = { _ in }
        AlphaLlamaCppProvider.draftAccelerationValidator = { _, _, _ in
            XCTFail("Constrained quick-start E4B should reject high-token draft before validation")
            return false
        }
        AlphaLlamaCppProvider.physicalMemoryBytesProvider = {
            8_400_000_000
    ...
    func testExperimentalGGUFProviderHidesDraftMetadataAfterStandardFallback() async throws {
        actor StubLlamaContext: AlphaLlamaCompletionContext {
            ...
            func completionLoop() -> String { "Standard fallback answer" }
            func isDone() -> Bool { true }
            func promptTokenCount() -> Int { 512 }
            func generatedTokenCount() -> Int { 18 }
            func accelerationMode() -> AlphaLocalRuntimeAccelerationMode { .standard }
        ...
        let mainURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("ross-gguf-main-standard-\(UUID().uuidString)")
            .appendingPathExtension("gguf")
        let draftURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("ross-gguf-draft-standard-\(UUID().uuidString)")
            .appendingPathExtension("gguf")
        try Data("gguf-main-runtime".utf8).write(to: mainURL)
        try Data("gguf-draft-runtime".utf8).write(to: draftURL)
        defer {
    ...
    @available(iOS 26.0, macOS 26.0, *)
    @MainActor
    func testActivePackAndRuntimeHealthUseRecoveredFallbackWhenSystemAssistantIsUnavailable() async throws {
        let previousDisableFlag = ProcessInfo.processInfo.environment["ROSS_DISABLE_DEVELOPMENT_MODEL_ARTIFACTS"]
        setenv("ROSS_DISABLE_DEVELOPMENT_MODEL_ARTIFACTS", "1", 1)
        defer {
            if let previousDisableFlag {
        ...
        let checksum = sha256Hex(data)
        let relativePath = "model-packs/case_associate/execution-fallback.gguf"
        let artifactURL = alphaAbsoluteURL(for: relativePath)
        let manifestURL = artifactURL.deletingPathExtension().appendingPathExtension("manifest.json")
        try FileManager.default.createDirectory(
            at: artifactURL.deletingLastPathComponent(),
    ...
    @available(iOS 26.0, macOS 26.0, *)
    @MainActor
    func testLocalAskPreflightUpgradeUsesInstalledSeniorGGUFBeforePromptingAnotherUpgrade() async throws {
        let previousDisableFlag = ProcessInfo.processInfo.environment["ROSS_DISABLE_DEVELOPMENT_MODEL_ARTIFACTS"]
        let previousSimulatorIdentifier = ProcessInfo.processInfo.environment["SIMULATOR_MODEL_IDENTIFIER"]
        let previousAvailabilityProbe = AlphaFoundationModelsLocalProvider.modelAvailabilityProbe
        let previousModelValidator = AlphaLlamaCppProvider.modelLoadValidator
        ...
        var availableMLXPack = try await installMLXPack(
            ...
            fileName: "gemma-4-12b-it-mlx",
            packId: "case-mlx",
            fixtureName: "ask-preflight-senior-mlx-\(UUID().uuidString)"
        )
        availableMLXPack.isActive = false
        var seniorGGUFPack = try await installGGUFPack(
            with: store,
            tier: .seniorDraftingSupport,
            fileName: "gemma-4-26B-A4B-it-UD-Q4_K_XL.gguf",
            packId: "senior-gguf"
        )
        seniorGGUFPack.isActive = false
        var systemPack = alphaSystemAssistantPack(for: .caseAssociate)
        systemPack.isActive = true

        let caseID = UUID()
        let documents = makeMatterAskDocuments(count: 14)
        let matter = AlphaCaseMatter(
            id: caseID,
            title: "Large file matter",
            forum: "High Court",
        ...
        var state = AlphaPersistedState.empty()
        state.settings.activeTier = .caseAssociate
        state.installedPacks = [systemPack, availableMLXPack, seniorGGUFPack]
        state.cases = [matter]
    ...
    @available(iOS 26.0, macOS 26.0, *)
    @MainActor
    func testSubmitAskUsesInstalledSeniorGGUFForOversizedMatterBundle() async throws {
        actor StubLlamaContext: AlphaLlamaCompletionContext {
            private let chunks: [String]
            private var emittedIndex = 0
            private var generatedTokens = 0
        ...
        AlphaMLXLocalProvider.streamGenerator = { _, _, _, _, _, _, _ in
            probeBox.usedMLX = true
            return AlphaMLXGenerationSnapshot(text: "MLX answer\n- This should not run first.")
        }
        AlphaLlamaCppProvider.contextFactory = { _, _, _ in
            probeBox.usedGGUF = true
            return StubLlamaContext(chunks: ["Senior answer", "\n- GGUF handled the larger matter bundle."])
        }
        AlphaLlamaCppProvider.modelLoadValidator = { _ in }
        AlphaLlamaCppProvider.draftAccelerationValidator = { _, _, _ in true }
        defer {
        ...
        var availableMLXPack = try await installMLXPack(
            ...
            fileName: "gemma-4-12b-it-mlx",
            packId: "case-mlx",
            fixtureName: "ask-runtime-senior-mlx-\(UUID().uuidString)"
        )
        availableMLXPack.isActive = false
        var seniorGGUFPack = try await installGGUFPack(
            with: store,
        ...
        XCTAssertTrue(probeBox.usedGGUF)
        XCTAssertFalse(probeBox.usedFoundation)
        XCTAssertFalse(probeBox.usedMLX)
        XCTAssertEqual(
            model.latestAskResult?.modelInvocation?.runtimeMode,
            AlphaPackRuntimeMode.llamaCppGguf.rawValue
        )
        XCTAssertEqual(
            model.latestAskResult?.modelInvocation?.capabilityTier,
            AlphaCapabilityTier.seniorDraftingSupport.rawValue
        )
    }
    #endif
    ...
    func testAssistantCatalogDescriptorFallsBackToPreferredRuntimeWhenTargetPackIdMissing() {
        let manifest = AlphaBackendCatalogManifest(
            packs: [
                AlphaBackendCatalogPack(
                    packId: "gemma-4-12b-mlx",
                    ...
                    tier: .caseAssociate,
                    sizeBytes: 6_200_000_000,
                    checksumSha256: String(repeating: "3", count: 64),
                    artifactKind: "mlx_directory",
                    runtimeMode: .mlxSwiftLm,
                    developmentOnly: false
                ),
                AlphaBackendCatalogPack(
                    ...
                    tier: .caseAssociate,
                    sizeBytes: 7_381_382_048,
                    checksumSha256: String(repeating: "4", count: 64),
                    artifactKind: "local_model_artifact",
                    runtimeMode: .llamaCppGguf,
                    developmentOnly: false
                )
    ...
    func testAssistantVariantOptionsIncludeInstalledAlternatesAndBuiltIn() {
        ...
        XCTAssertEqual(options.first?.detailLabel, "~13 tok/s · 24,576 tokens · Companion")
        XCTAssertTrue(options.contains(where: { $0.runtimeMode == .llamaCppGguf && !$0.isBuiltIn }))
        XCTAssertTrue(options.contains(where: { $0.runtimeMode == .llamaCppGguf && $0.detailLabel == "~11 tok/s · 28,672 tokens · MTP" }))
    ...
    func testAssistantVariantOptionsAvoidDuplicateBuiltInWhenAlreadyInstalled() {
        ...
        XCTAssertEqual(options.dropFirst().first?.detailLabel, "~13 tok/s · 24,576 tokens · Companion")
    ...
    func testInstalledPackRuntimeSummaryUsesMeasuredMLXSpeedOnRecentIPhone() {
        ...
        XCTAssertEqual(
            ...
            "MLX · 13 tok/s · 28,672 tokens · Companion"
        )
    }
    ...
    func testInstalledPackRuntimeSummaryUsesEstimatedGGUFSpeedWhenNoRecentRun() {
        ...
        XCTAssertEqual(
            ...
            "Gemma GGUF · ~10 tok/s · 28,672 tokens · MTP"
        )
    }
    ...
    func testAssistantSetupPresentationUsesBundledMLXWhenPreferredMetadataIsMissing() {
        ...
        XCTAssertEqual(presentation?.runtimeMode, .mlxSwiftLm)
        XCTAssertEqual(presentation?.totalDownloadBytes, 11_290_232_154)
        XCTAssertEqual(presentation?.sizeLabel, alphaAssistantStorageSizeLabel(11_290_232_154))
        XCTAssertEqual(presentation?.speedLabel, "~13 tok/s")
        XCTAssertEqual(presentation?.contextLabel, "24,576 tokens")
        XCTAssertEqual(presentation?.companionLabel, alphaAssistantSetupCompanionLabel(for: .mlxSwiftLm))
    ...
    func testAssistantSetupCompactSummaryLabelIncludesRuntimeAndSize() {
        let downloadPresentation = AlphaAssistantSetupPresentation(
            ...
            companionLabel: alphaAssistantSetupCompanionLabel(for: .mlxSwiftLm),
            etaLabel: "about 19 min"
        )
        XCTAssertEqual(
            alphaAssistantSetupCompactSummaryLabel(downloadPresentation),
            "MLX · 13.4 GB · Companion"
        )

        XCTAssertEqual(
            alphaAssistantSetupCompanionLabel(for: .llamaCppGguf),
            "MTP draft acceleration"
        )
    ...
    func testMatterQuestionBudgetPlannerKeepsMoreSingleSelectedLlamaContext() {
        let plan = AlphaLocalPromptBudgetPlanner.matterQuestionPlan(
            runtimeMode: .llamaCppGguf,
            baseMaxInputChars: 52_000,
            sourceBlockCount: 16,
    ...
    func testMatterQuestionBudgetPlannerUsesResolvedHighBudgetFoundationBranch() {
        let plan = AlphaLocalPromptBudgetPlanner.matterQuestionPlan(
            runtimeMode: .appleFoundationModels,
            baseMaxInputChars: 64_736,
            sourceBlockCount: 14,
            sourceCharCount: 52_000,
            lastInvocation: nil
        )

        XCTAssertEqual(plan.maxInputChars, 58_262)
        XCTAssertEqual(plan.sourceBlockLimit, 12)
        XCTAssertEqual(plan.sourceExcerptChars, 1_900)
    }

    func testMatterQuestionBudgetPlannerWidensResolvedHighBudgetFoundationBranchAfterFastRun() {
        let fastInvocation = AlphaLocalModelInvocation(
            task: .matterQuestionAnswer,
            runtimeMode: AlphaPackRuntimeMode.appleFoundationModels.rawValue,
            caseId: nil,
            ...
            promptHash: "prompt",
            inputHash: "input",
            estimatedOutputTokensPerSecond: 16.8,
            timeToFirstTokenMs: 980,
            status: .complete
        )
        ...
        XCTAssertEqual(plan.maxInputChars, 69_914)
        XCTAssertEqual(plan.sourceBlockLimit, 14)
        XCTAssertEqual(plan.sourceExcerptChars, 2_040)
    }
    ...
    func testMatterQuestionBudgetPlannerKeepsMoreSingleSelectedHighBudgetFoundationContext() {
        let plan = AlphaLocalPromptBudgetPlanner.matterQuestionPlan(
            runtimeMode: .appleFoundationModels,
            baseMaxInputChars: 64_736,
            sourceBlockCount: 24,
    ...
    func testStructuredDocumentBudgetPlannerUsesResolvedHighBudgetFoundationBranch() {
        let plan = AlphaLocalPromptBudgetPlanner.structuredDocumentPlan(
            runtimeMode: .appleFoundationModels,
            baseMaxInputChars: 64_736,
            sourceBlockCount: 18,
        ...
        XCTAssertEqual(plan.maxInputChars, 56_967)
        XCTAssertEqual(plan.sourceBlockLimit, 15)
        XCTAssertEqual(plan.sourceExcerptChars, 1_720)
    }

    func testStructuredDocumentBudgetPlannerWidensResolvedHighBudgetFoundationBranchAfterFastRun() {
        let fastInvocation = AlphaLocalModelInvocation(
            task: .legalFieldExtraction,
            runtimeMode: AlphaPackRuntimeMode.appleFoundationModels.rawValue,
            caseId: nil,
        ...
        XCTAssertEqual(plan.maxInputChars, 69_267)
        XCTAssertEqual(plan.sourceBlockLimit, 17)
        XCTAssertEqual(plan.sourceExcerptChars, 1_760)
    }
    ...
    func testStructuredDocumentBudgetPlannerKeepsMoreSingleSelectedHighBudgetFoundationContext() {
        let plan = AlphaLocalPromptBudgetPlanner.structuredDocumentPlan(
            runtimeMode: .appleFoundationModels,
            baseMaxInputChars: 64_736,
            sourceBlockCount: 24,
    ...
    func testStructuredDocumentBatchLimitUsesMidHighMLXBranch() {
        let batchLimit = AlphaLocalPromptBudgetPlanner.structuredDocumentBatchLimit(
            runtimeMode: .mlxSwiftLm,
            capabilityTier: .caseAssociate,
            task: .legalFieldExtraction,
    ...
    func testStructuredDocumentBatchLimitWidensFurtherAfterFastMidHighMLXRun() {
        let fastInvocation = AlphaLocalModelInvocation(
            task: .legalFieldExtraction,
            runtimeMode: AlphaPackRuntimeMode.mlxSwiftLm.rawValue,
            caseId: nil,
            ...
            promptHash: "prompt",
            inputHash: "input",
            estimatedOutputTokensPerSecond: 18.1,
            timeToFirstTokenMs: 1_150,
            status: .complete
        )
    ...
    func testStructuredDocumentBatchLimitUsesResolvedHighBudgetFoundationBranch() {
        let batchLimit = AlphaLocalPromptBudgetPlanner.structuredDocumentBatchLimit(
            runtimeMode: .appleFoundationModels,
            capabilityTier: .caseAssociate,
            task: .legalFieldExtraction,
    ...
    func testStructuredDocumentBatchLimitWidensFurtherAfterFastHighBudgetFoundationRun() {
        let fastInvocation = AlphaLocalModelInvocation(
            task: .legalFieldExtraction,
            runtimeMode: AlphaPackRuntimeMode.appleFoundationModels.rawValue,
            caseId: nil,
    ...
    func testStructuredDocumentBatchLimitWidensCaseMemoryAfterFastMidHighMLXRun() {
        let fastInvocation = AlphaLocalModelInvocation(
            task: .caseMemorySynthesis,
            runtimeMode: AlphaPackRuntimeMode.mlxSwiftLm.rawValue,
            caseId: nil,
    ...
    func testAskRuntimeSourcePackPolicyUsesResolvedHighBudgetFoundationBand() {
        let policy = alphaAskRuntimeSourcePackPolicy(
            runtimeMode: .appleFoundationModels,
            capabilityTier: .caseAssociate,
            baseMaxInputChars: 64_736,
    ...
    func testAskRuntimeSourcePackPolicyUsesResolvedHighBudgetSingleSelectedFoundationBand() {
        let policy = alphaAskRuntimeSourcePackPolicy(
            runtimeMode: .appleFoundationModels,
            capabilityTier: .caseAssociate,
            baseMaxInputChars: 64_736,
    ...
    func testAskRuntimeSourcePackPolicyUsesResolvedHighBudgetFoundationCandidateWindow() {
        let policy = alphaAskRuntimeSourcePackPolicy(
            runtimeMode: .appleFoundationModels,
            capabilityTier: .caseAssociate,
            baseMaxInputChars: 64_736,
