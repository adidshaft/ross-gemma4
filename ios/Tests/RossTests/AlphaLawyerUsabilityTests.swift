...
final class AlphaLawyerUsabilityTests: XCTestCase {
    ...
    func testAssistantModelUpdatesIgnoreStaleCachedCatalogFallbackUntilRefreshCompletes() async throws {
        rossSetBackendBaseURLOverride("http://127.0.0.1:9")
        defer { rossSetBackendBaseURLOverride(nil) }

        try await withRestoredStore { store in
            let artifact = alphaAssistantModelArtifact(for: .caseAssociate)
            let relativePath = "model-packs/case_associate/stale-cached-update.gguf"
            let artifactURL = alphaAbsoluteURL(for: relativePath)
            let manifestURL = artifactURL.deletingPathExtension().appendingPathExtension("manifest.json")
            try FileManager.default.createDirectory(
                at: artifactURL.deletingLastPathComponent(),
            ...
            state.lastModelCatalogRefresh = Date().addingTimeInterval(-(86_400 + 60))
            state.modelUpdateCandidates = []
            try await store.replace(with: state)
    ...
    func testPrivateAIDeviceComparisonProofStatusesTrackSavedCoverageByTarget() {
        ...
        XCTAssertEqual(alphaPrivateAIDeviceComparisonMissingTargetLabels([saved8GBRecord, simulatorRecord]), ["12 GB+ class"])
        XCTAssertEqual(
    ...
    func testMatterBundleComparisonExportBodyLinesIncludeReadoutAndLatestRuntimeDetails() {
        ...
        XCTAssertTrue(joined.contains("28 GB free"))
        XCTAssertTrue(joined.contains("Low Power Mode: No"))
        XCTAssertTrue(joined.contains("Thermal state: Nominal"))
