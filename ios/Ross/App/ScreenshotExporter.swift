        ...
        if smokeProfile.isShortGenerationProfile {
            if sourceBoundOutput.schemaValid,
               ...
               sourceKeptRefs {
                ...
                status = RossLocalModelSmokeStatusCopy.failedStatus
                RossLocalModelSmokeView.log("ROSS_LOCAL_MODEL_SMOKE_FAIL runtime=\(provider.runtimeMode.rawValue) requested_runtime=\(runtimeEnvironment.runtimeModeOverride?.rawValue ?? "nil") tier=\(activePack.tier.rawValue) profile=\(smokeProfile.rawValue) elapsed=\(String(format: "%.2f", elapsed))s source_error=\(sourceBoundOutput.errorCategory ?? "nil") general_error=\(generalOutput.errorCategory ?? "nil") source_grounded=\(sourceUsedFileFact) source_refs_kept=\(sourceKeptRefs) source_refs=\(sourceBoundOutput.sourceRefs.count) source_native_model=\(sourceNativeModel) general_native_model=\(generalNativeModel) source_warning_count=\(sourceBoundOutput.warnings.count) general_warning_count=\(generalOutput.warnings.count) source_raw_chars=\(sourceRawLength) general_output_chars=\(generalOutputLength) \(sourceBenchmarkFields) \(generalBenchmarkFields)")
