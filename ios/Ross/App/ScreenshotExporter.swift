...
func alphaDebugSmokeResolvedModelURL(for modelPath: String) -> URL {
    ...
    return alphaAbsoluteURL(for: modelPath)
}
...
struct RossLocalModelSmokeView: View {
    ...
    private func runSmoke() async {
        ...
        let shortDraftStageError = RossLocalModelSmokeView.draftStageAcceptanceError(
            requireDraftAcceleration: requireDraftAcceleration,
            stageOutputs: [
                ("source", sourceBoundOutput),
        ...
        if smokeProfile.isShortGenerationProfile {
            if sourceBoundOutput.schemaValid,
               ...
               sourceUsedFileFact,
               sourceKeptRefs,
               shortDraftStageError == nil {
                status = RossLocalModelSmokeStatusCopy.passedStatus
                RossLocalModelSmokeView.log("ROSS_LOCAL_MODEL_SMOKE_PASS runtime=\(provider.runtimeMode.rawValue) requested_runtime=\(runtimeEnvironment.runtimeModeOverride?.rawValue ?? "nil") tier=\(activePack.tier.rawValue) profile=\(smokeProfile.rawValue) elapsed=\(String(format: "%.2f", elapsed))s source_raw_chars=\(sourceRawLength) source_parsed_chars=\(sourceParsedLength) general_output_chars=\(generalOutputLength) source_refs=\(sourceBoundOutput.sourceRefs.count) source_native_model=\(sourceNativeModel) general_native_model=\(generalNativeModel) \(sourceBenchmarkFields) \(generalBenchmarkFields)")
            } else {
                status = RossLocalModelSmokeStatusCopy.failedStatus
                RossLocalModelSmokeView.log("ROSS_LOCAL_MODEL_SMOKE_FAIL runtime=\(provider.runtimeMode.rawValue) requested_runtime=\(runtimeEnvironment.runtimeModeOverride?.rawValue ?? "nil") tier=\(activePack.tier.rawValue) profile=\(smokeProfile.rawValue) elapsed=\(String(format: "%.2f", elapsed))s source_error=\(sourceBoundOutput.errorCategory ?? "nil") general_error=\(generalOutput.errorCategory ?? "nil") draft_stage_error=\(shortDraftStageError ?? "nil") source_grounded=\(sourceUsedFileFact) source_refs_kept=\(sourceKeptRefs) source_refs=\(sourceBoundOutput.sourceRefs.count) source_native_model=\(sourceNativeModel) general_native_model=\(generalNativeModel) source_warning_count=\(sourceBoundOutput.warnings.count) general_warning_count=\(generalOutput.warnings.count) source_raw_chars=\(sourceRawLength) general_output_chars=\(generalOutputLength) \(sourceBenchmarkFields) \(generalBenchmarkFields)")
        ...
        let fullBenchmarkFields = RossLocalModelSmokeView.benchmarkFields(
            stageOutputs: [
                ("source", sourceBoundOutput),
                ("bengali", bengaliOutput),
                ("hindi", hindiOutput),
                ("tamil", tamilOutput),
                ("telugu", teluguOutput),
                ("general", generalOutput),
        ...
        let fullDraftStageError = RossLocalModelSmokeView.draftStageAcceptanceError(
            requireDraftAcceleration: requireDraftAcceleration,
            stageOutputs: [
                ("source", sourceBoundOutput),
                ("bengali", bengaliOutput),
        ...
        RossLocalModelSmokeView.log("ROSS_LOCAL_MODEL_SMOKE_STAGE final_validation profile=\(smokeProfile.rawValue)")
        if sourceBoundOutput.schemaValid,
           sourceBoundOutput.errorCategory == nil,
    ...
    nonisolated static func benchmarkFields(stage: String, output: AlphaLocalModelOutput) -> String {
        ...
        let fields: [(String, String)] = [
            ...
            ("\(prefix)_draft_failure", output.speculativeDraftFailureReason.map(stableSmokeValue) ?? "nil"),
    ...
    nonisolated static func benchmarkFields(stageOutputs: [(stage: String, output: AlphaLocalModelOutput)]) -> String {
        stageOutputs
            .map { benchmarkFields(stage: $0.stage, output: $0.output) }
            .joined(separator: " ")
    }

    nonisolated static func draftStageAcceptanceError(
        requireDraftAcceleration: Bool,
        stageOutputs: [(stage: String, output: AlphaLocalModelOutput)]
    ) -> String? {
        guard requireDraftAcceleration else { return nil }
