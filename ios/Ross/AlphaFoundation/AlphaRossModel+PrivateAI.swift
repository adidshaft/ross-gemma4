...
private let alphaBundledMLXQuickStartAssistantDownloadDescriptor = AlphaAssistantDownloadDescriptor(
    ...
    checksumSha256: "2da1fd6bb6401c3ef116ac921dca88f73e4901a80ab10a4e8b21563412dbe23c",
...
private let alphaBundledMLXQuickStartAssistantDraftDownloadDescriptor = AlphaAssistantDownloadDescriptor(
    ...
    checksumSha256: "6c3171c6ce77320de39ab8eb3a206daecd221b7dfb965ec689ae450bf868c8f3",
    artifactKind: "mlx_directory",
    runtimeMode: .mlxSwiftLm,
    developmentOnly: false,
...
private func alphaMLXArtifactRequiresUnsupportedRuntime(
    packId: String? = nil,
    fileName: String? = nil,
    downloadURLString: String? = nil
) -> Bool {
    let joined = [packId, fileName, downloadURLString]
        .compactMap { $0?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() }
        .joined(separator: " ")
    guard !joined.isEmpty else { return false }
    if joined.contains("optiq") { return true }

    let unsupportedPrimaryArchives = [
        "gemma-4-e4b-it-qat-4bit",
        "gemma-4-12b-it-qat-4bit",
        "gemma-4-26b-a4b-it-qat-4bit",
        "gemma-4-31b-it-qat-4bit"
    ]
    return unsupportedPrimaryArchives.contains { archiveName in
        joined.contains(archiveName) && !joined.contains("\(archiveName)-assistant")
    }
}
...
func alphaAssistantDraftArtifactSupportsCurrentInstaller(_ artifact: AlphaAssistantDraftArtifactDescriptor) -> Bool {
    ...
    case .llamaCppGguf:
        return artifact.fileName.lowercased().hasSuffix(".gguf") &&
            artifact.artifactKind.localizedCaseInsensitiveContains("local_model_artifact")
    case .mlxSwiftLm:
        guard !alphaMLXArtifactRequiresUnsupportedRuntime(
...
func alphaAssistantCatalogDescriptorSupportsCurrentInstaller(_ descriptor: AlphaAssistantCatalogDescriptor) -> Bool {
    ...
    case .mlxSwiftLm:
        guard !alphaMLXArtifactRequiresUnsupportedRuntime(packId: descriptor.packId) else {
            return false
        }
...
func alphaBackendCatalogPackSupportsCurrentInstaller(_ pack: AlphaBackendCatalogPack) -> Bool {
    ...
    case .mlxSwiftLm:
        guard !alphaMLXArtifactRequiresUnsupportedRuntime(packId: pack.packId) else {
            return false
        }
